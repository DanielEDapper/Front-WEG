async function searchLocation(cep)
{
    let errorMessage = getElementById('erro');
    errorMessage.innerHTML = '';

    const URL = `https://viacep.com.br/ws/${cep}/json/`;

    try {
        let searchCEP = await fetch(URL);
        let searchConvertedCEP = await searchCEP.json();

        if(searchConvertedCEP.erro)
        {
            throw new Error("CEP NÃO FOI LOCALIZADO");
        }

        let city = document.getElementById('cidade');
        let publicSpace = document.getElementById('endereco');
        let state = document.getElementById('estado');

        city.value = searchConvertedCEP.localidade;
        publicSpace.value = searchConvertedCEP.logradouro;
        state.value = searchConvertedCEP.uf;

        console.log(searchConvertedCEP);
        return searchConvertedCEP;
    }
    catch(erro)
    {
        errorMessage.innerHTML = `<p> ERROU BOBÃO </p>`
        console.error(erro);
    }
}

let cep = document.getElementById('cep');
cep.addEventListener('focusout', () => searchLocation(cep.value));
...
// .finally(mensagem => console.log("O processamento foi concluido com sucesso"))
