...
const createElement = (event) => {
    ...
    const content = `<p class="info-produto" data-preco="${inputPriceValue}"><strong>${inputNameValue}</strong> - R$ ${inputPriceValue.toFixed(2)}</p>
    ...
    totalValue += inputPriceValue;

    priceElement.textContent = `R$ ${totalValue.toFixed(2)}`;
}
...
list.addEventListener("click", (event) => {
  if (event.target.classList.contains("btn-remover")) {
    ...
    const info = item.querySelector(".info-produto");
    const infoValue = parseFloat(info.dataset.preco);
